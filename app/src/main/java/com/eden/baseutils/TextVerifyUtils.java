...
public class TextVerifyUtils {
     ...
     * 验证文本是否是手机号
     *
     * @param phoneText 文本内容
     * @return 返回值 true/false
     * @describe 移动：134、135、136、137、138、139、150、151、157(TD)、158、159、187、188
     * 联通：130、131、132、152、155、156、185、186
     * 电信：133、153、180、189、（1349卫通）
     * 总结起来就是第一位必定为1，第二位必定为3或5或8，其他位置的可以为0-9
     */
    public static boolean isMobile(String phoneText) {
        String num = "[1][3578]\\d{9}";//"[1]"代表第1位为数字1，"[358]"代表第二位可以为3、5、8中的一个，"\\d{9}"代表后面是可以是0～9的数字，有9位。
        if (TextUtils.isEmpty(phoneText)) {
            return false;
        } else {
            //matches():字符串是否在给定的正则表达式匹配
            return phoneText.matches(num);
        }
    }
     ...
     * @return true/false
     */
    public static boolean isPhoneOrEmail(String text) {
        return isMobile(text) || isEmail(text);
    }
    ...
    public static boolean IDCardValidate(String IDStr) {
        ...
        // ================ 出生年月是否有效 ================
        int strYear = Integer.parseInt(Ai.substring(6, 10));// 年份
        int strMonth = Integer.parseInt(Ai.substring(10, 12));// 月份
        int strDay = Integer.parseInt(Ai.substring(12, 14));// 日
        ...
        if ((nowYear - strYear) > 150 || !((strYear > nowYear) || (strYear == nowYear && strMonth > nowMonth) || (strYear == nowYear && strMonth == nowMonth && strDay > nowDay))) {
